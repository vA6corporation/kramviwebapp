...
export class RemissionGuidesService {
    ...
    addRemissionGuideItem(product: ProductModel) {
        const index = this.remissionGuideItems.findIndex(e => e.productId === product.id)
        if (index < 0) {
    ...
    getRemissionGuidesBySale(saleId: any): Observable<RemissionGuideModel[]> {
        return this.httpService.get(`remissionGuides/bySale/${saleId}`)
    }

    getRemissionGuideById(remissionGuideId: any): Observable<RemissionGuideModel> {

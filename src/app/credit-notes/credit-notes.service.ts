...
export class CreditNotesService {
    ...
    getCdrByCreditNote(creditNoteId: any): Observable<CdrNcModel> {
        return this.httpService.get(`creditNotes/cdrxml/${creditNoteId}`)
    }

    updateDate(creditNote: any, creditNoteId: any): Observable<void> {
        return this.httpService.put(`creditNotes/updateDate/${creditNoteId}`, { creditNote })
    }

    cancelCreditNote(creditNoteId: any, reason: string) {
    ...
    addCreditNoteItem(product: ProductModel) {
        const index = this.creditNoteItems.findIndex(e => e.productId === product.id && e.igvCode === product.igvCode)
        if (index < 0) {
    ...
    getById(creditId: any) {
        return this.httpService.get(`creditNotes/byId/${creditId}`)
    }

    getCreditNotesBySale(saleId: any): Observable<CreditNoteModel[]> {
    ...
    getCreditNoteById(creditNoteId: any): Observable<CreditNoteModel> {
    ...
    getCreditNotesByPage(
        ...
        return this.httpService.get(`creditNotes/byPage/${pageIndex}/${pageSize}`, params)
    }

    getCountCreditNotes(params: Params): Observable<number> {
        return this.httpService.get(`creditNotes/countCreditNotes`, params)
    }

    sendCreditNote(creditNoteId: any): Observable<CreditNoteModel> {
    ...
    deleteBySale(saleId: any): Observable<void> {
        return this.httpService.delete(`creditNotes/bySale/${saleId}`)
    }

    delete(creditNoteId: any): Observable<void> {

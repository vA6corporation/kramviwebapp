...
export class DetailPurchasesComponent {
    ...
    onDeletePaymentPurchase(paymentPurchaseId: any, purchaseId: any) {
    ...
    onEditPaymentPurchase(paymentPurchase: PaymentPurchaseModel) {
        ...
        dialogRef.afterClosed().subscribe(updatePayment => {
            if (updatePayment) {
                this.navigationService.loadBarStart()
                this.paymentPurchasesService.update(updatePayment, paymentPurchase.id, paymentPurchase.purchaseId).subscribe({
                    next: () => {

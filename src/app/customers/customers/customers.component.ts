...
export class CustomersComponent {
        ...
        this.handleClickMenu$ = this.navigationService.handleClickMenu().subscribe(async id => {
            switch (id) {
                case 'export_excel': {
                    ...
                    const name = `CLIENTES_${formatDate(new Date(), 'dd/MM/yyyy', 'en-US')}_${this.business.name.replace(/ /g, '_').toUpperCase()}`
    ...
    onOpenDialogProducts() {
        ...
        dialogRef.afterClosed().subscribe(product => {
            if (product) {
                Object.assign(this.params, { productId: product.id })
                this.fetchData()
                this.fetchCount()
            }
        })
    ...
    onCheckAllCustomers(isChecked: boolean) {
        if (isChecked) {
            this.customerIds = this.$dataSource().map(e => e.id)
        } else {
            this.customerIds = []
        }
    }

    onCheckCustomer(isChecked: boolean, customerId: any) {
        if (isChecked) {

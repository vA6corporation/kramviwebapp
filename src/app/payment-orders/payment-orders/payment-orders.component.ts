...
export class PaymentOrdersComponent {
                            ...
                            next: paymentOrders => {
                                ...
                                for (const paymentOrder of paymentOrders) {
                                    body.push([
                                        ...
                                        paymentOrder.charge,
                                        paymentOrder.serie,
                                        (paymentOrder.observation || '').toUpperCase(),
                                        formatDate(paymentOrder.paymentAt, 'dd/MM/yyyy', 'en-US'),
                                        paymentOrder.operationNumber,
                                        paymentOrder.isPaid ? 'SI' : 'NO',
                                    ])
                                }
                                const name = `ORDENES_DE_PAGO_${formatDate(new Date(), 'dd/MM/yyyy', 'en-US')}_${this.business.name.replace(/ /g, '_')}`
    ...
    onShowPdf(paymentOrder: PaymentOrderModel) {
        ...
        dialogRefPDf.componentInstance.handleDeletePdf().subscribe(() => {
            this.navigationService.loadBarStart()
            this.paymentOrdersService.deleteFile(paymentOrder.id).subscribe(() => {
                this.navigationService.loadBarFinish()
                paymentOrder.urlPdf = ''
            })
        })

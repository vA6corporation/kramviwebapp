...
export class ExcelConcar {
    ...
    addSales(
        ...
        let officeSales = this.officeSales[office.id]
        if (officeSales) {
            this.officeSales[office.id] = [...officeSales, ...sales]
        } else {
            this.officeSales[office.id] = sales
        }
    }
    ...
    buildBody(
        ...
        sales.forEach((sale, index) => {
            ...
            this.body.push([
                ...
                this.formatDateSlash(sale.createdAt),
                'MN',
                ...
                '',
                'D',
                sale.cdr && sale.cdr.sunatCode === '0' && sale.deletedAt === null ? Number(sale.charge.toFixed(2)) : 0,
                0,
                0,
            ...
            this.body.push([
                ...
                '',
                'H',
                sale.cdr && sale.cdr.sunatCode === '0' && sale.deletedAt === null ? Number(sale.igv.toFixed(2)) : 0,
                0,
                0,
                sale.invoiceCode === '01' ? 'FT' : 'BV',
                `${sale.invoicePrefix}${office.serialPrefix}-${sale.invoiceNumber}`,
            ...
            this.body.push([
                ...
                '',
                'H',
                sale.cdr && sale.cdr.sunatCode === '0' && sale.deletedAt === null ? Number((sale.charge - sale.igv).toFixed(2)) : 0,
                0,
                0,
                sale.invoiceCode === '01' ? 'FT' : 'BV',
                `${sale.invoicePrefix}${office.serialPrefix}-${sale.invoiceNumber}`,
    ...
    builExcel() {
        ...
        for (const [key, sales] of Object.entries(this.officeSales)) {
            const foundOffice = this.offices.find(e => e.id.toString() === key)
            if (foundOffice) {
                sales.reverse()
    ...
    private formatSerie(sale: SaleModel, index: number) {
        ...
        var month = new Date(sale.createdAt).getMonth() + 1

...
export async function buildA4CreditNote(
    ...
    const header = 11
    const body = 8
    const { creditNoteItems, customer, user, sale } = creditNote
   ...
   // }
    ...
    pdf.setFont('Helvetica', 'normal')
    text = creditNote?.observation || 'NINGUNO'
    ...
    text = formatDate(creditNote?.createdAt || '', 'dd/MM/yyyy', 'en-US')
    pdf.text(text, 160, 50)

    text = `${formatDate(creditNote?.createdAt || '', 'h:mm a', 'en-US')}`
    ...
    for (const creditNoteItem of creditNoteItems || []) {
        ...
        if (creditNoteItem.observation) {
            text = `S/N: ${creditNoteItem.observation}`
            strArr = pdf.splitTextToSize(text, 90)
            pdf.text(strArr, 47, positionYitems)
...
async function getQRDataUrl(creditNote: CreditNoteModel, business: BusinessModel, office: OfficeModel): Promise<string> {
    return await QRCode.toDataURL(`${business.ruc}|${creditNote.invoiceName.toUpperCase()}|${creditNote.invoicePrefix}${office.serialPrefix}|${creditNote.invoiceNumber}|${creditNote.igv.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}|${creditNote.charge.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}|${creditNote.createdAt}`, { margin: 0 })
}

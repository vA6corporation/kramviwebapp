...
export async function buildTicket80mmTwo(
    sale: SaleModel,
    ...
    const { saleItems, customer, user, payments, worker } = sale
    ...
    for (const saleItem of saleItems) {
        strArr = pdf.splitTextToSize(`${saleItem.fullName.toUpperCase()}${saleItem.observations ? ' - ' + saleItem.observations : ''}`, 70)
        pdf.text(strArr, 0 + marginLeft, positionY)
        positionY += 3 * strArr.length
    ...
    if (sale.invoiceType !== 'NOTA DE VENTA' || setting.showQrOnTicket) {
        ...
        pdf.addImage(qrcode, "JPEG", 24, positionY, 22, 22)
        positionY += 29
    }
...
async function getQRDataUrl(sale: SaleModel, business: BusinessModel, office: OfficeModel): Promise<string> {
    return await QRCode.toDataURL(`${business.ruc}|${sale.invoiceType}|${sale.invoicePrefix}${office.serialPrefix}|${sale.invoiceNumber}|${sale.igv.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}|${sale.charge.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}|${sale.createdAt}`, { margin: 0 })
}
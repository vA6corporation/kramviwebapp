...
export function buildTurn80mm(
    ...
    strArr = pdf.splitTextToSize(turn.observation || '', 72)
    ...
    for (const summarySaleItem of summarySaleItems) {
        ...
        pdf.text((summarySaleItem.totalCharge / summarySaleItem.totalQuantity).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }), 35, positionY, { align: 'center' })
        pdf.text(summarySaleItem.totalCharge.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }), 70 - marginRight, positionY, { align: 'right' })
        positionY += 4
    }

...
export class PrintIframeComponent {
    ...
    ngOnInit(): void {
            ...
            if (authStatus) {
                ...
                this.handleAuth$ = this.authService.handleAuth().subscribe(auth => {
                    ...
                    this.settingsService.getLogo(auth.setting.uuid).then(blobLogo => {
                        this.urlLogo = URL.createObjectURL(blobLogo)
                    })
                })
        ...
        this.printService.handlePrintA4Invoice().subscribe(async sale => {
            const pdf = await buildA4Invoice(sale, this.setting, this.business, this.office, this.banks, this.paymentMethods, this.urlLogo)
            if (main) {
                const file = pdf.output('arraybuffer')
                main.print(file)
            } else {
        ...
        this.printService.handlePrintA4Proforma().subscribe(async proforma => {
            const pdf = await buildA4Proforma(proforma, this.setting, this.business, this.office, this.banks, this.urlLogo)
            if (main) {
                const file = pdf.output('arraybuffer')
                main.print(file)
            } else {
        ...
        this.printService.handleExportPdfTicket80mm().subscribe(async sale => {
            const pdf = await buildTicket80mm(sale, this.setting, this.business, this.office, this.banks, this.paymentMethods, this.urlLogo)
            pdf.save(`${sale.invoicePrefix}${this.office.serialPrefix}-${sale.invoiceNumber}`)
        })
        ...
        this.printService.handlePrintCommandFastFood80mm().subscribe(async sale => {
            if (main) {
                const printers = this.printers
                if (printers.length) {
                    for (const printer of printers) {
                        if (printer.printKitchen) {
                            const saleItems = sale.saleItems.filter(e => e.printZone === PrintZoneType.COCINA)
                            if (saleItems.length) {
                                const pdf = buildCommandFastFood80mm(sale, saleItems, this.office, this.setting, this.paymentMethods)
                                const file = pdf.output('arraybuffer')
                                main.print(file, printer.name)
                            }
                        }
                        ...
                        if (printer.printBar) {
                            const saleItems = sale.saleItems.filter(e => e.printZone === PrintZoneType.BARRA)
                            if (saleItems.length) {
                                const pdf = buildCommandFastFood80mm(sale, saleItems, this.office, this.setting, this.paymentMethods)
                                const file = pdf.output('arraybuffer')
                                main.print(file, printer.name)
                        ...
                        if (printer.printOven) {
                            const saleItems = sale.saleItems.filter(e => e.printZone === PrintZoneType.HORNO)
                            if (saleItems.length) {
                                const pdf = buildCommandFastFood80mm(sale, saleItems, this.office, this.setting, this.paymentMethods)
                                const file = pdf.output('arraybuffer')
                                main.print(file, printer.name)
                        ...
                        if (printer.printBox) {
                            const saleItems = sale.saleItems.filter(e => e.printZone === PrintZoneType.CAJA)
                            if (saleItems.length) {
                                const pdf = buildCommandFastFood80mm(sale, saleItems, this.office, this.setting, this.paymentMethods)
                                const file = pdf.output('arraybuffer')
                                main.print(file, printer.name)
                    ...
                    const pdf = buildCommandFastFood80mm(sale, sale.saleItems, this.office, this.setting, this.paymentMethods)
                    const file = pdf.output('arraybuffer')
                    main.print(file)
                }
            } else {
    ...
    print(blobUrl: URL) {
        this.printIframe.nativeElement.contentWindow?.location.replace(blobUrl)
    }

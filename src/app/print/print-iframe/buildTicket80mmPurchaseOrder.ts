...
export async function buildTicket80mmPurchaseOrder(
    ...
    text = `O${office?.serialPrefix}-${purchaseOrder?.purchaseOrderNumber}`
    pdf.text(text, pageCenter + marginLeft, positionY, { align: 'center' })
    positionY += 4
    ...
    for (const purchaseOrderItem of purchaseOrderItems) {
        strArr = pdf.splitTextToSize(`${purchaseOrderItem.fullName.toUpperCase()}${purchaseOrderItem.observation ? ' - ' + purchaseOrderItem.observation : ''}`, 70)
        pdf.text(strArr, 0 + marginLeft, positionY)
        positionY += 3 * strArr.length
    ...
    if (purchaseOrder.observation) {
        text = purchaseOrder.observation

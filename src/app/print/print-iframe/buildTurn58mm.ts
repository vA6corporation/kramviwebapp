...
export function buildTurn58mm(
    ...
    strArr = pdf.splitTextToSize(turn.observation || '', 47)
    ...
    for (const summarySaleItem of summarySaleItems) {
        ...
        pdf.text(`${Number((summarySaleItem.totalCharge / summarySaleItem.totalQuantity).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }))}`, 25, positionY)
        pdf.text(summarySaleItem.totalCharge.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }), 47, positionY, { align: 'right' })
        positionY += 4
    }

...
export function buildPreaccount80mm(
    ...
    positionY += 3
    pdf.text(`IMPORTE TOTAL: ${board.boardItems.filter(e => e.igvCode !== IgvCode.BONIFICACION).map(e => e.price * e.quantity).reduce((a, b) => a + b, 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, 1 + marginLeft, positionY)
    positionY += 5

...
import { DialogDetailOutIncidentsComponent } from '../dialog-detail-out-incidents/dialog-detail-out-incidents.component'
...
@Component({
    ...
    templateUrl: './out-incidents.component.html',
    styleUrl: './out-incidents.component.sass'
})
export class OutIncidentsComponent {
    ...
    displayedColumns: string[] = ['created', 'product', 'quantity', 'user', 'observation', 'actions']
        ...
        this.handleClickMenu$ = this.navigationService.handleClickMenu().subscribe(id => {
            ...
            if (startDate && endDate) {
                ...
                for (let index = 0; index < this.$length() / chunk; index++) {
                    const promise = lastValueFrom(this.incidentsService.getOutIncidentItemsByPage(index + 1, chunk, this.params))
    ...
    onCreateIncident() {
        this.matBottomSheet.open(SheetIncidentsComponent)
    }

    onDetailIncident(incidentItemId: string) {
        this.matDialog.open(DialogDetailOutIncidentsComponent, {
            width: '600px',
            position: { top: '20px' },
            data: incidentItemId,
        })
    ...
    fetchData() {
        this.navigationService.loadBarStart()
        this.incidentsService.getOutIncidentItemsByPage(
            this.pageIndex + 1,
            this.pageSize,
            this.params
    ...
    onDeleteIncident(incidentId: string, incidentItemId: string) {
        ...
        if (ok) {
            this.navigationService.loadBarStart()
            this.incidentsService.deleteOutIncidentItem(incidentId, incidentItemId).subscribe(() => {
                this.navigationService.loadBarFinish()

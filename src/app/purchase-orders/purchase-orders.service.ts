...
export class PurchaseOrdersService {
    ...
    handlePurchaseOrderItems(): Observable<PurchaseOrderItemModel[]> {
    ...
    addPurchaseOrderItem(product: ProductModel) {
        const index = this.purchaseOrderItems.findIndex(e => e.productId === product.id && e.igvCode === product.igvCode)
        if (index < 0) {
            const purchaseOrderItem: PurchaseOrderItemModel = {
                sku: null,
                fullName: product.fullName,
    ...
    getPurchaseOrderById(purchaseOrderId: any): Observable<PurchaseOrderModel> {

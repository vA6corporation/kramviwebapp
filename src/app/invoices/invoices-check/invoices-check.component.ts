...
export class InvoicesCheckComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.handleSearch$ = this.navigationService.handleSearch().subscribe((key: string) => {
            ...
            this.salesService.getSalesByKey(key).subscribe(sales => {
    ...
    async excelConcar() {
        ...
        //     return 0;
        // });
        // const excelConcar = new ExcelConcar(startDate, endDate, this.business);
        // for (const office of offices) {
        //     const sales: SaleModel[] = await lastValueFrom(this.salesService.getSalesByRangeDateOfficeTax(startDate, endDate, office._id));
    ...
    async excelSimple() {
        ...
        for (let index = 0; index < this.length / chunk; index++) {
            const values = await lastValueFrom(this.salesService.getSalesByRangeDatePage(startDate, endDate, index + 1, chunk, {}))
            dialogRef.componentInstance.onComplete()
            sales.push(...values)
        }
    ...
    onSendInvoice(saleId: string) {
        this.navigationService.loadBarStart();
        this.invoicesService.sendInvoice(saleId).subscribe(cdr => {
            this.fetchData();
            this.navigationService.showMessage('Enviado a sunat');
            this.navigationService.loadBarFinish();
        }, (error: HttpErrorResponse) => {

...
export class CreditsService {
    ...
    getCreditById(creditId: any): Observable<CreditModel> {
        return this.httpService.get(`credits/byId/${creditId}`)
    }

    getCreditsByCustomer(customerId: any): Observable<CreditModel[]> {
        return this.httpService.get(`credits/byCustomer/${customerId}`)
    }

    getCountCreditsByTurn(turnId: any): Observable<number> {
        return this.httpService.get(`credits/countByTurn/${turnId}`)
    }

    getCreditsByPageTurn(pageIndex: number, pageSize: number, turnId: any): Observable<CreditModel[]> {
        return this.httpService.get(`credits/byPageTurn/${pageIndex}/${pageSize}/${turnId}`)
    }

    getCreditsByTurn(turnId: any): Observable<CreditModel[]> {
